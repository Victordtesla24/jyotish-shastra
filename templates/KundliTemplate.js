/**
 * KundliTemplate.js - Consolidated Kundli Template (Standalone JavaScript)
 * ------------------------------------------------------------
...
/**
 * Generate SVG markup for Kundli chart
 * @param {Object} chartData - Birth chart JSON data
 * @param {Object} options - Configuration options
 * @returns {string} - SVG markup
 */
function generateKundliSVG(chartData, options = {}) {
  const { 
    width = SIZE, 
    height = SIZE, 
  ...
  try {
    const { planets, rashis, rasiGlyphs } = processChartData(chartData);
    ...
    let svg = `<svg width="${width}" height="${height}" viewBox="0 0 ${SIZE} ${SIZE}" class="${className}">`;
    ...
    // Background square
    svg += `<rect x="${PAD}" y="${PAD}" width="${SIZE - 2 * PAD}" height="${SIZE - 2 * PAD}" class="bg stroke" />`;

    // Corner‑to‑corner diagonals
    svg += `<line x1="${TL.x}" y1="${TL.y}" x2="${BR.x}" y2="${BR.y}" class="stroke" />`;
    svg += `<line x1="${TR.x}" y1="${TR.y}" x2="${BL.x}" y2="${BL.y}" class="stroke" />`;

    // Large diamond
    svg += `<polyline points="${midTop.x},${midTop.y} ${midRight.x},${midRight.y} ${midBottom.x},${midBottom.y} ${midLeft.x},${midLeft.y} ${midTop.x},${midTop.y}" class="stroke" />`;

    // Rashi numbers
    Object.entries(HOUSE_CENTRES).forEach(([houseNum, center]) => {
      svg += `<text x="${center.x}" y="${center.y - 9}" class="rashi">${rashis[parseInt(houseNum) - 1]}</text>`;
    });

    // Planets
    planets.forEach((planet, index) => {
      svg += `<text x="${planet.x}" y="${planet.y}" class="planet">${planet.code}${planet.dignity} ${planet.deg}°</text>`;
    });

    // Rasi glyphs
    rasiGlyphs.forEach((glyph, index) => {
      svg += `<text x="${glyph.x}" y="${glyph.y}" class="rasi-glyph">${glyph.glyph}</text>`;
    });
    ...
    // Return error SVG
    return `<svg width="${width}" height="${height}" viewBox="0 0 ${SIZE} ${SIZE}" class="${className}-error">
      <rect x="${PAD}" y="${PAD}" width="${SIZE - 2 * PAD}" height="${SIZE - 2 * PAD}" 
            fill="#fee" stroke="#f00" stroke-width="2" />
      <text x="${SIZE/2}" y="${SIZE/2}" text-anchor="middle" dominant-baseline="middle" 
            fill="#f00" font-size="12" font-family="sans-serif">
        Error: ${error.message}
      </text>
    </svg>`;
  }
...
/**
 * Create a DOM element with the Kundli chart
 * @param {Object} chartData - Birth chart JSON data
 * @param {Object} options - Configuration options
 * @returns {HTMLElement} - DOM element containing the SVG
 */
function createKundliElement(chartData, options = {}) {
  const { containerClass = "kundli-container", ...svgOptions } = options;
  
  const container = document.createElement('div');
  container.className = containerClass;
  container.innerHTML = generateKundliSVG(chartData, svgOptions);
  
  return container;
}

/**
 * Render Kundli chart to a DOM element
 * @param {string|HTMLElement} target - Target element or selector
 * @param {Object} chartData - Birth chart JSON data
 * @param {Object} options - Configuration options
 */
function renderKundliToElement(target, chartData, options = {}) {
  ...
  element.innerHTML = generateKundliSVG(chartData, options);
}
...
function validateChartData(data) {
  ...
  try {
    ...
    if (!data?.data?.rasiChart) {
      errors.push("Missing data.rasiChart structure");
      return { valid: false, errors, warnings };
    }
    
    const rasiChart = data.data.rasiChart;
    
    // Ascendant validation
    if (!rasiChart.ascendant?.signId && rasiChart.ascendant?.signIndex === undefined) {
      errors.push("Missing ascendant information");
    }
    ...
    if (!rasiChart.planets || !Array.isArray(rasiChart.planets)) {
      ...
      rasiChart.planets.forEach((planet, index) => {
        if (!planet.name) warnings.push(`Planet ${index}: Missing name`);
        if (!planet.signId) warnings.push(`Planet ${index}: Missing signId`);
        if (planet.degree === undefined) warnings.push(`Planet ${index}: Missing degree`);
      });
    }
    
    // House positions validation
    if (!rasiChart.housePositions || !Array.isArray(rasiChart.housePositions)) {
      warnings.push("Missing housePositions array - will use calculated positions");
    }
