...
# Render Deployment API Endpoint Testing Script
# Tests all endpoints against Render deployment URL
# Usage: ./scripts/test-render-endpoints.sh [RENDER_URL]
# Default: Uses RENDER_URL environment variable or prompts

if [ -z "$1" ]; then
  if [ -z "$RENDER_URL" ]; then
    echo "Error: Render URL not provided."
    echo "Usage: ./scripts/test-render-endpoints.sh <RENDER_URL>"
    echo "   or: RENDER_URL=<url> ./scripts/test-render-endpoints.sh"
    exit 1
  fi
...
echo "=========================================="
echo "Render Deployment API Endpoint Testing"
echo "=========================================="
...
test_endpoint() {
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    ...
    if [ "$success" = "true" ] || [ -n "$(echo "$body" | jq -r 'select(.status) | .status' 2>/dev/null)" ]; then
      ...
      ((PASSED++))
      TEST_RESULTS+=("✅ $name: PASSED (HTTP $http_code)")
    else
      echo "  ⚠️  Status: WARNING - Response structure may differ (HTTP $http_code)"
      TEST_RESULTS+=("⚠️  $name: WARNING (HTTP $http_code)")
    fi
  else
...
test_endpoint "API Information" "GET" "${BASE_URL%/v1}/" ""
