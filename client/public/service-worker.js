/* Jyotish Shastra Service Worker
 * Enables offline functionality and caching for better performance
 */

const CACHE_NAME = 'jyotish-shastra-v1';
const urlsToCache = [
  '/',
  '/index.html',
  '/manifest.json',
...
self.addEventListener('sync', event => {
  if (event.tag === 'sync-birth-data') {
    event.waitUntil(syncBirthData());
  }
});

async function syncBirthData() {
  // Sync pending birth data when back online
  console.log('Syncing birth data...');
}

// Push notifications for daily horoscopes
self.addEventListener('push', event => {
  const options = {
    body: event.data ? event.data.text() : 'Your daily horoscope is ready!',
    icon: '/favicon.svg',
    badge: '/favicon.svg',
    vibrate: [100, 50, 100],
    data: {
